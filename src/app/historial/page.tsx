        ...
        {diagnosticos.length === 0 ? (
          ...
          <div className="space-y-3">
            {diagnosticos.map(d => {
              ...
              return (
                <div key={d.id} onClick={() => router.push(`/resultados/${d.id}`)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0"
                        style={{ backgroundColor: `${color}20`, color }}>{d.nivelGlobal}</span>
                    </div>
                    ...
                    <div className="flex gap-1.5 mt-2">
                      {d.resultados.map(r => (
                        <div key={r.dimensionId} className="flex-1 h-1.5 rounded-full bg-gray-100 overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${r.puntaje}%`, backgroundColor: r.color }} />
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={e => handleEliminar(e, d.id)}
